...
module.exports = {
	...
	createpayament (payament) {
		return _database.zunpc.model.epayament.create(payament);
	},

	createToken (datatoken) {
		return _database.zunpc.model.enzonatoken.create(datatoken);
	},
	...
	getToken () {
		...
		let sql = `
		select * from enzonatoken ORDER BY identoken DESC LIMIT 1
		`;
		let options = {
	...
	update (objecttoken) {
		return _database.zunpc.model.enzonatoken.update({
			access_token: objecttoken.access_token,
			scope: objecttoken.scope,
			token_type: objecttoken.token_type,
			expires_in: objecttoken.expires_in,
			fecha: objecttoken.fecha
		},{
			where: {
				identoken: objecttoken.identoken
			}
		});
	},
